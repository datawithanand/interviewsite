 ...
 */
async function notify({ userId, actorId = null, type, message, relatedQuestionId = null, relatedModuleId = null }) {
  if (!userId || userId === actorId) return; // don't notify users about their own actions
  try {
    await prisma.notification.create({
      data: { userId, actorId, type, message, relatedQuestionId, relatedModuleId },
    });
  } catch (err) {
